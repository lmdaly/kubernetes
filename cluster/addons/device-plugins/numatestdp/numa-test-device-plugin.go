...
func (test *testManager) discoverTestResources() error {
 	...
 	glog.Infof("Discovered Devices below:")
 		dev001 := pluginapi.Device{ID: "25102017", Health: pluginapi.Healthy, Topology: &pluginapi.TopologyInfo{Node: &pluginapi.NUMANode{ID: 0,},}}
 	dev002 := pluginapi.Device{ID: "26102017", Health: pluginapi.Healthy, Topology: &pluginapi.TopologyInfo{Node: &pluginapi.NUMANode{ID: 1,},}}
        dev003 := pluginapi.Device{ID: "27102017", Health: pluginapi.Healthy,Topology: &pluginapi.TopologyInfo{Node: &pluginapi.NUMANode{ID: 1,},}}
	...
	for {
		...
		if changed {
			...
			for _, dev := range test.devices {
				resp.Devices = append(resp.Devices, &pluginapi.Device{ID: dev.ID, Health: dev.Health, Topology: &pluginapi.TopologyInfo{Node: dev.Topology.Node}})
			}
...
func (test *testManager) Allocate(ctx context.Context, rqt *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
	...
	for _, container := range rqt.ContainerRequests {
		...
		for _, id := range container.DevicesIDs {
			...
			ids = ids + "{" + id + ", " + strconv.Itoa(int(dev.Topology.Node.ID)) + "}"
		}
