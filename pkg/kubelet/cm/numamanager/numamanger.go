...
func getNumaAffinityStrings (num1, num2 string) (string, error) {

	// Get bit lengths
	l1, l2 := len(num1), len(num2)
	val1, err := strconv.ParseUint(num1, 2, 32)
	if err != nil {
		return "", err
	}

	val2, err := strconv.ParseUint(num2, 2, 32)
	if err != nil {
		return "", err
	}

	// Only do right-shift if the representation length aren't uqual
	if l1 != l2 {
		// Get the bit difference
		diff := math.Abs(float64(l1) - float64(l2))
		if l1 < l2 {
	...
	andValue := val1 & val2
	return strconv.FormatInt(int64(andValue), 2), nil
}
